...
my_file.add_command(label="New", compound=tk.LEFT,
                    accelerator="(Ctrl+N)", command=new_file)
my_file.add_command(label="Open", compound=tk.LEFT,
...
my_edit.add_command(label="Cut", compound=tk.LEFT, accelerator="(Ctrl+X)",
                    command=lambda: text_editor.event_generate("<Control x>"))
my_edit.add_command(label="Copy", compound=tk.LEFT, accelerator="(Ctrl+C)",
                    command=lambda: text_editor.event_generate("<Control c>"))
my_edit.add_command(label="Paste", compound=tk.LEFT, accelerator="(Ctrl+V)",
                    command=lambda: text_editor.event_generate("<Control v>"))
my_edit.add_separator()
my_edit.add_command(label="Clear All", compound=tk.LEFT,
                    accelerator="(Cltr+Alt+X)", command=lambda: text_editor.delete(1.0, tk.END))
        ...
        if word:
            ...
            while True:
                ...
                text_editor.tag_config(
                    "match", foreground="white", background="black")
...
my_edit.add_command(label="Find", compound=tk.LEFT,
                    accelerator="(Cltr+F)", command=find_func)
...
my_view.add_checkbutton(label="Status Bar",  onvalue=True, offvalue=0,
                        variable=show_status_bar, compound=tk.LEFT, command=hide_stat)
...
for i in color_dict:
    my_color.add_radiobutton(label=i, compound=tk.LEFT,
                             variable=theme_select, command=change_theme)
...
font_size = ttk.Combobox(tool_bar_label, width=20,
                         state="readonly", textvariable=size_var)
